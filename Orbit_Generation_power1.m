function[total_calfay]= Orbit_Generation_power1(nu,e,i,Om,om,epoch1,time_end,Hx1)
...
figure (2)
x= plot (xx,calfaxpi,'r')
xlim([0 max(xx)])
hold on
y=plot (xx,calfaypi,'b')
xlim([0 max(xx)])
hold on
z= plot (xx,calfazpi,'k')
xlim([0 max(xx)])
hold on
 plot (xx, calfaxni,'r')
xlim([0 max(xx)])
...
legend([x y z],'x','y','z','Location','NorthEast');
hold off
